...
class OpsKPIStrategy(KPIStrategy):
    ...
    def get_card_config(self, data_context):
        ...
        return {
            "title": self.title, "icon": self.icon, "color": self.color, "inverse": self.inverse,
            "value": node.get("value_formatted", "---"), "main_value": node.get("value_formatted", "---"),
            "vs_last_year_formatted": node.get("vs_last_year_formatted"), "vs_last_year_delta": node.get("vs_last_year_delta"),
            "vs_last_year_delta_formatted": node.get("vs_last_year_delta_formatted"), "label_prev_year": node.get("label_prev_year", "Año Ant."),
            "target_formatted": node.get("target_formatted"), "target_delta": node.get("target_delta"),
            "target_delta_formatted": node.get("target_delta_formatted"), "trend": node.get("trend"),
            "trend_direction": node.get("trend_direction"), "ytd_formatted": node.get("ytd_formatted"),
            "ytd_delta": node.get("ytd_delta"), "ytd_delta_formatted": node.get("ytd_delta_formatted"),
            "status": node.get("status"), "status_color": node.get("status_color")
        }
...
class OpsGaugeStrategy(KPIStrategy):
    ...
    def get_card_config(self, data_context):
        ...
        if not node:
            return {"title": self.title, "main_value": "---", "icon": self.icon, "color": self.color, "raw_data": []}
        return {
            "title": self.title, "icon": self.icon, "color": self.color, "value": node.get("value_formatted", "---"),
            "main_value": node.get("value_formatted", "---"), "vs_last_year_formatted": node.get("vs_last_year_formatted"),
            "vs_last_year_delta": node.get("vs_last_year_delta"), "vs_last_year_delta_formatted": node.get("vs_last_year_delta_formatted"),
            "target_formatted": node.get("target_formatted"), "target_delta": node.get("target_delta"),
            "target_delta_formatted": node.get("target_delta_formatted"), "ytd_formatted": node.get("ytd_formatted"),
            "ytd_delta": node.get("ytd_delta"), "ytd_delta_formatted": node.get("ytd_delta_formatted"), "status": node.get("status")
        }
    ...
    def get_figure(self, data_context, theme="dark"):
        ...
        fig.add_trace(go.Indicator(
            ...
            gauge={"axis": {"range": [0, max_val * 1.1], "tickwidth": 1}, "bar": {"color": gauge_color},
                   "bgcolor": Colors.TRANSPARENT, "borderwidth": 0,
                   "threshold": {"line": {"color": Colors.CHART_TARGET, "width": 4}, "thickness": 0.8, "value": target if target else max_val}}
        ))
        if target:
            fig.add_annotation(x=0.5, y=-0.15, text=f"<b>META:</b> {target:,.0f}", showarrow=False,
                font=dict(size=Typography.BADGE, color=Colors.CHART_TARGET, family=Typography.FAMILY), xref="paper", yref="paper")
        if vs_last_year:
            fig.add_annotation(x=0.5, y=-0.30, text=f"Año {prev_year}: {vs_last_year:,.0f}", showarrow=False,
                font=dict(size=Typography.XS, color=Colors.CHART_PREVIOUS, family=Typography.FAMILY), xref="paper", yref="paper")
        fig.update_layout(template="plotly_dark" if theme == "dark" else "plotly",
            height=self.layout.get("height", ComponentSizes.KPI_HEIGHT_NORMAL), margin=dict(l=25, r=25, t=30, b=50), hovermode=False)
        return fig
...
class OpsTrendChartStrategy(KPIStrategy):
    ...
    def get_figure(self, data_context, theme="dark"):
        ...
        for idx, s in enumerate(series_list):
            ...
            if "Meta" in s_name:
                fig.add_trace(go.Scatter(x=categories, y=s_data, name="Meta", mode='lines+markers',
                    line=dict(color=base_color, width=2, dash='dot'), marker=dict(size=6)))
            else:
                fig.add_trace(go.Bar(x=categories, y=past_y, name=s_name.replace(curr_year, "Actual").replace(prev_year, "Anterior"),
                    marker=dict(color=base_color, cornerradius=6), width=0.38, offsetgroup=str(idx)))
                fig.add_trace(go.Bar(x=categories, y=future_y, name=f"{s_name} (proy.)",
                    marker=dict(color=base_color, opacity=0.45, cornerradius=6), width=0.38, offsetgroup=str(idx), showlegend=False))
        h_val = self.layout.get("height")
        plot_height = h_val if isinstance(h_val, (int, float)) else ComponentSizes.CHART_HEIGHT_BASE
        fig.update_layout(template="plotly_dark" if theme == "dark" else "plotly",
            height=plot_height, barmode='group', hovermode='x unified',
            legend=dict(orientation="h", yanchor="bottom", y=1.05, xanchor="center", x=0.5))
        return fig

class OpsDonutChartStrategy(KPIStrategy):
    def __init__(self, screen_id, chart_key, title, icon="tabler:chart-pie", color="indigo", color_map=None, has_detail=True, layout_config=None):
        super().__init__(title=title, icon=icon, color=color, has_detail=has_detail, layout_config=layout_config)
        self.screen_id = screen_id
        self.chart_key = chart_key
    ...
    def get_figure(self, data_context, theme="dark"):
        ...
        fig = go.Figure(data=[go.Pie(labels=labels, values=values, hole=0.65,
            ...
            textinfo="none", hovertemplate="<b>%{label}</b><br>%{value:,.0f}<br>%{percent}<extra></extra>",
            pull=[0.05 if i == max_index else 0 for i in range(len(values))])])
        if total:
            fig.add_annotation(text=f"<b>Total</b><br>{total}", x=0.30, y=0.5, showarrow=False,
        ...
        fig.update_layout(template="plotly_dark" if theme == "dark" else "plotly",
            height=fig_height, margin=dict(t=30, b=20, l=10, r=10), showlegend=True,
...
class OpsHorizontalBarStrategy(KPIStrategy):
    ...
    def get_figure(self, data_context, theme="dark"):
        ...
        fig.add_trace(go.Bar(x=values, y=categories, orientation='h',
            marker=dict(color=Colors.CHART_GOLD, cornerradius=6), width=0.80,
            text=[f"${v/1e6:.1f}M" if v >= 1e6 else f"${v/1e3:.1f}k" if v >= 1000 else f"${v:.0f}" for v in values],
            textposition='outside', hovertemplate='<b>%{y}</b><br>Valor: $%{x:,.2f}<extra></extra>'))
        if target_val > 0 or avg_value > 0:
            ref_val = target_val if target_val > 0 else avg_value
            fig.add_vline(x=ref_val, line_width=2, line_dash="dot", line_color=Colors.CHART_TARGET,
        ...
        fig.update_layout(template="plotly_dark" if theme == "dark" else "plotly",
            height=calculated_height, margin=dict(t=50, b=40, l=120, r=80), showlegend=False,
            xaxis=dict(showgrid=True, zeroline=False, tickformat='$,.0f', side="top"),
            yaxis=dict(type='category', autorange="reversed", automargin=True), bargap=0.8)
        return fig
...
class OpsTableStrategy:
    ...
    def _get_data(self, data_context):
        ...
        for i, h in enumerate(headers_list):
            columns_config.append({"field": f"col_{i}", "headerName": h})
        safe_row_data = []
        for row in raw_rows:
            row_dict = {}
    ...
    def get_card_config(self, data_context):
        ...
        return {"title": self.title or self.table_key, "icon": self.icon, "is_table": True,
                "main_value": f"{len(row_data)} registros" if row_data else "0", "raw_data": export_data}
    ...
    def _render_dashboard(self, columns_config, row_data, theme="dark"):
        ...
        column_defs = []
        for i, col in enumerate(columns_config):
            col_def = {"field": col["field"], "headerName": col["headerName"], "sortable": True,
                       "resizable": True, "suppressMenu": True, "tooltipField": col["field"]}
            if i == 0:
                col_def.update({"pinned": "left", "width": 90, "flex": 0})
            else:
                col_def.update({"minWidth": 130, "flex": 1})
            column_defs.append(col_def)
        grid = dag.AgGrid(id={"type": "ag-grid-dashboard", "index": unique_key}, rowData=row_data, columnDefs=column_defs,
            defaultColDef={"sortable": True, "resizable": True, "filter": False},
            dashGridOptions={"pagination": True, "paginationPageSize": 15, "rowHeight": ComponentSizes.TABLE_ROW_HEIGHT,
                             "headerHeight": ComponentSizes.TABLE_HEADER_HEIGHT, "suppressFieldDotNotation": True, "quickFilterText": ""},
            style={"height": "100%", "width": "100%"}, className="ag-theme-alpine compact")
        search_bar = dmc.Group(justify="space-between", mt=Space.SM, children=[
            dmc.TextInput(id={"type": "ag-quick-search", "index": unique_key}, placeholder="Buscar...",
                leftSection=DashIconify(icon="tabler:search", width=ComponentSizes.ICON_SM), size="xs", radius="xl", style={"width": "220px"},
                styles={"input": {"fontSize": f"{Typography.SM}px", "height": f"{ComponentSizes.BUTTON_HEIGHT_SM}px",
            ...
            dmc.Text(f"{len(row_data)} registros", size="xs", c="dimmed",
        ...
        return html.Div(style={"height": "100%", "display": "flex", "flexDirection": "column"}, children=[
            html.Div(style={"flex": 1, "minHeight": "250px", "overflow": "hidden"}, children=grid), search_bar])
    ...
    def _render_analyst(self, columns_config, row_data, theme="dark"):
        ...
        for i, col in enumerate(columns_config):
            col_def = {"field": col["field"], "headerName": col["headerName"], "sortable": True,
                       "filter": _get_filter_type(col["field"]), "resizable": True, "floatingFilter": True,
                       "minWidth": 110, "flex": 1, "tooltipField": col["field"]}
            if i == 0:
                col_def["pinned"] = "left"
            column_defs.append(col_def)
        grid = dag.AgGrid(id=table_id, rowData=row_data, columnDefs=column_defs,
            dashGridOptions={"pagination": True, "paginationPageSize": 50, "suppressFieldDotNotation": True},
            style={"height": "100%", "width": "100%"}, className="ag-theme-alpine")
...
class OpsMapStrategy(KPIStrategy):
    ...
    def get_figure(self, data_context, theme="dark"):
        ...
        for route in routes:
            ...
            if lats and lons:
                fig.add_trace(go.Scattermapbox(lat=lats, lon=lons, mode='lines+markers', name=name,
        ...
        fig.update_layout(mapbox=dict(style="open-street-map", center=dict(lat=center_lat, lon=center_lon), zoom=10),
            margin=dict(l=0, r=0, t=0, b=0), height=self.layout.get("height", ComponentSizes.CHART_HEIGHT_LG), showlegend=True,
            font=dict(family=Typography.FAMILY, size=Typography.SM, color=Colors.TEXT_DARK if is_dark else Colors.TEXT_LIGHT),
            legend=dict(yanchor="top", y=0.99, xanchor="left", x=0.01, bgcolor=legend_bg,

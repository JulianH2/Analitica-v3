...
class ExecutiveKPIStrategy(KPIStrategy):
    def __init__(self, screen_id, kpi_key, title, icon, color, is_pct=False, has_detail=True, layout_config=None, inverse=False, variant=None):
...
class ExecutiveMiniKPIStrategy(KPIStrategy):
    def __init__(self, screen_id, kpi_key, title, color, icon, layout_config=None, variant=None):
        super().__init__(title=title, color=color, icon=icon, has_detail=True, layout_config=layout_config, variant=variant)
        self.screen_id = screen_id
        self.kpi_key = kpi_key
    ...
    def get_card_config(self, data_context):
        ...
        if not node:
            return {"title": self.title, "value": "---", "icon": self.icon, "color": self.color}

        val = node.get("value") or node.get("current_value") or node.get("valor", 0)
        formatted = node.get("value_formatted") or format_value(val, "$", format_type="abbreviated") # type: ignore

        return {"title": self.title, "value": formatted, "icon": self.icon, "color": self.color}

    def render_detail(self, data_context):
        return None

class ExecutiveDonutStrategy(KPIStrategy):
    def __init__(self, screen_id, chart_key, title, icon="tabler:chart-pie", color_map=None, has_detail=True, layout_config=None, variant=None):
        super().__init__(title=title, has_detail=has_detail, icon=icon, layout_config=layout_config, variant=variant)
        self.screen_id = screen_id

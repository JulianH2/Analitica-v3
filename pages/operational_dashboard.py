...
w_inc = SmartWidget(f"{PREFIX}_inc", OpsGaugeStrategy(screen_id=SCREEN_ID, key="revenue_total", title="Ingreso Viaje", icon="tabler:cash", color="indigo", has_detail=True, layout_config={"height": 300}))
w_tri = SmartWidget(f"{PREFIX}_tri", OpsGaugeStrategy(screen_id=SCREEN_ID, key="total_trips", title="Viajes", icon="tabler:truck", color="green", has_detail=True, layout_config={"height": 300}))
w_kms = SmartWidget(f"{PREFIX}_kms", OpsGaugeStrategy(screen_id=SCREEN_ID, key="total_kilometers", title="Kilómetros", icon="tabler:route", color="yellow", has_detail=True, layout_config={"height": 300}))

w_avg_trip = SmartWidget(f"{PREFIX}_avg_trip", OpsKPIStrategy(screen_id=SCREEN_ID, key="revenue_per_trip", title="Prom. x Viaje", icon="tabler:calculator", color="blue", has_detail=True))
w_avg_unit = SmartWidget(f"{PREFIX}_avg_unit", OpsKPIStrategy(screen_id=SCREEN_ID, key="revenue_per_unit", title="Prom. x Unidad", icon="tabler:truck-delivery", color="indigo", has_detail=True))
w_units_qty = SmartWidget(f"{PREFIX}_units", OpsKPIStrategy(screen_id=SCREEN_ID, key="units_used", title="Unidades Uso", icon="tabler:packages", color="violet", has_detail=True))
w_customers = SmartWidget(f"{PREFIX}_customers", OpsKPIStrategy(screen_id=SCREEN_ID, key="customers_served", title="Clientes", icon="tabler:users", color="teal", has_detail=True))

class DynamicTrendChartStrategy(OpsTrendChartStrategy):
    def __init__(self, screen_id, key, base_title, icon="tabler:chart-line", color="indigo", has_detail=True, layout_config=None):
        self.base_title = base_title
        super().__init__(screen_id, key, get_dynamic_title(base_title), icon, color, has_detail, layout_config)
...
c_inc = ChartWidget(f"{PREFIX}_inc_comp", DynamicTrendChartStrategy(screen_id=SCREEN_ID, key="revenue_trends", base_title="Ingresos", icon="tabler:chart-line", color="indigo", has_detail=True, layout_config={"height": 340}))
c_trips = ChartWidget(f"{PREFIX}_trips_comp", DynamicTrendChartStrategy(screen_id=SCREEN_ID, key="trips_trends", base_title="Viajes", icon="tabler:chart-line", color="green", has_detail=True, layout_config={"height": 340}))

c_mix = ChartWidget(f"{PREFIX}_mix", OpsDonutChartStrategy(screen_id=SCREEN_ID, key="revenue_by_operation_type", title="Ingreso por Tipo Operación", color="indigo", has_detail=True, layout_config={"height": 360}))
c_unit = ChartWidget(f"{PREFIX}_unit_bal", OpsHorizontalBarStrategy(screen_id=SCREEN_ID, key="revenue_by_unit", title="Balanceo Ingresos por Unidad", has_detail=True, layout_config={"height": 340}))
...
def _render_fleet_status(ctx):
    ...
    if val < 1:
        val *= 100
    if val >= 80:
        color, icon_color = "green", DesignSystem.SUCCESS[5]
    elif val >= 60:
        color, icon_color = "yellow", DesignSystem.WARNING[5]
    else:
        color, icon_color = "red", DesignSystem.DANGER[5]
    return dmc.Paper(
        p=10,
        withBorder=True,
        radius="md",
        style={"height": "390px", "backgroundColor": "transparent"},
        children=[dmc.Stack(justify="center", align="center", style={"height": "100%"}, gap=8, children=[
...
def _render_routes_tabs(ctx, theme):
    return dmc.Paper(
        ...
        radius="md",
        style={"height": "390px", "backgroundColor": "transparent"},
        children=[dmc.Tabs(value="rutas_cargado", children=[
            dmc.TabsList([
...
def _render_income_tabs(ctx, theme):
    return dmc.Paper(
        p=8,
        withBorder=True,
        children=[dmc.Tabs(value="ingreso_cliente", children=[
            dmc.TabsList([
...
def _render_body(ctx):
    ...
    return html.Div([
        html.Div(style={"display": "grid", "gridTemplateColumns": "repeat(auto-fit, minmax(250px, 1fr))", "gap": "0.8rem", "marginBottom": "1rem"}, children=[
        ...
        html.Div(style={"display": "grid", "gridTemplateColumns": "repeat(auto-fit, minmax(400px, 1fr))", "gap": "0.8rem", "marginBottom": "1rem"}, children=[
        ...
        dmc.Grid(gutter="md", mb="lg", children=[
            dmc.GridCol(span={"base": 12, "md": 5}, children=[c_mix.render(ctx, theme=theme)]), # type: ignore
            dmc.GridCol(span={"base": 12, "md": 7}, children=[c_unit.render(ctx, theme=theme)]), # type: ignore
        ]),

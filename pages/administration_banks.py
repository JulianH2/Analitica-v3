...
w_init_con = SmartWidget(f"{PREFIX}_init_con", AdminKPIStrategy(screen_id=SCREEN_ID, key="initial_balance", title="Saldo Inicial Consolidado", icon="tabler:wallet", color="gray", variant="consolidado"))
...
def _banks_tab_content(ctx, variant, theme):
    if variant == "consolidado":
        k1, k2, k3, k4, c1, c2, t1 = w_init_con, w_inc_con, w_exp_con, w_fin_con, c_daily_con, c_donut_con, t_concepts_con
    elif variant == "pesos":
        k1, k2, k3, k4, c1, c2, t1 = w_init_mxn, w_inc_mxn, w_exp_mxn, w_fin_mxn, c_daily_mxn, c_donut_mxn, t_concepts_mxn
    else:
        k1, k2, k3, k4, c1, c2, t1 = w_init_usd, w_inc_usd, w_exp_usd, w_fin_usd, c_daily_usd, c_donut_usd, t_concepts_usd

    return [
        html.Div(style={"display": "grid", "gridTemplateColumns": "repeat(auto-fit, minmax(200px, 1fr))", "gap": "0.8rem", "marginBottom": "1.5rem"}, children=[
            k1.render(ctx, theme=theme),
            k2.render(ctx, theme=theme),
            k3.render(ctx, theme=theme),
            k4.render(ctx, theme=theme)
        ]),
